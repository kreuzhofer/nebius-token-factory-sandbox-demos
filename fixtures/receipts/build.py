...
def receipt_pdf(spec, hide_merchant=False):
    ...
    for number, lines in enumerate(spec['pages'], start=1):
        ...
        pdf.drawCentredString(WIDTH / 2, HEIGHT - 25,
                             'SYNTHETIC DEMO - NOT A REAL PURCHASE')
        ...
        for label, value in lines:
            pdf.setFont('Courier-Bold' if label.startswith(('TOTAL', 'SUMME', 'REFUND')) else 'Courier', 10)
        ...
        pdf.drawCentredString(WIDTH / 2, 35,
                             f"Page {number} of {len(spec['pages'])} - {spec['id'].upper()}")
        pdf.showPage()
    pdf.save()
...
def file_record(input_id, receipt_id, path, kind, language, coverage, **extra):
    ...
    return {'input_id': input_id, 'source_receipt_id': receipt_id,
...
def main():
    ...
    coverage = {'r01': ['photograph', 'low_resolution'],
    ...
    for source in public:
        ...
        records.append(file_record(source['receipt_id'], source['receipt_id'], path,
                                   'public', source['language'], coverage[source['receipt_id']],
    ...
    for spec in specs:
        ...
        records.append(file_record(spec['id'], spec['id'], path, 'synthetic',
                                   spec['language'], spec['coverage'],
    ...
    variants = [
        ...
        ('v02', 'v02-control-reencoded.png', ['reencoded_duplicate'], 'PNG compression level 0; decoded pixels equal s08.'),
        ...
        ('v05', 'v05-control-corrupt.pdf', ['corrupt_file'], 'Deliberately truncated PDF with no document structure.'),
        ('v06', 'v06-control-missing-merchant.png', ['missing_merchant'], 'Render s08 with its merchant heading masked; other visible values preserved.')
    ]
    for vid, name, tags, transformation in variants:
        ...
        records.append(file_record(vid, 's08', path, 'variant', 'en', tags,
                                   derived_from='s08', transformation=transformation,
    ...
    manifest = {'fixture_version': '1',
                ...
                'profiles': {'base': [r['input_id'] for r in records if r['kind'] != 'variant'],
    ...
    expected = {'schema_version': '1', 'scope': 'Lightweight intended behaviors; no model run or accuracy claim.',
                'synthetic': {s['id']: {'currency': s['currency'], 'transaction_type': s['type'], **s['expected']} for s in specs},
                'variants': {'v01': 'Count once with s08, retaining both source entries.',
                ...
                'public': {'r01': 'Do not invent a date outside the cropped evidence.',
    ...
    print(f'Built {len(specs)} synthetic receipts, {len(variants)} variants; {len(records)} total inputs.')
