...
def main():
    ...
    parser.add_argument(
        "--list", metavar="PROFILE", help="Print absolute input paths for a manifest profile"
    )
    ...
    assert Counter(r["kind"] for r in records.values()) == {
        "public": 4,
        "synthetic": 8,
        "variant": 6,
    }
    ...
    assert expected_files == {
        p.relative_to(ROOT).as_posix() for p in (ROOT / "inputs").iterdir() if p.is_file()
    }
    ...
    assert (records["v03"]["width"], records["v03"]["height"]) == (
        records["s08"]["height"],
        records["s08"]["width"],
    )
    ...
    for source in sources:
        ...
        if rid == "s04":
            ...
            assert all(
                value not in ("42.50", "42,50") for page in source["pages"] for _, value in page
            )
        else:
    ...
    print(
        "OK: 12 receipts + 6 variants; hashes, image decoding, PDF text/rendering, expected corruption, duplicate identity and synthetic arithmetic checked."
    )
