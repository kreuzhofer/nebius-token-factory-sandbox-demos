...
class LifecycleTests(unittest.TestCase):
    def test_poll_until_success(self):
        ...
        with patch.object(api, 'request', side_effect=[{'status': 'EXECUTING'}, {'status': 'SUCCESS'}]), patch('demo.time.sleep'):
    ...
    def test_process_failure_even_when_operation_succeeds(self):
        api = demo.API('test', 'project')
        with patch.object(api, 'request', return_value={'uuid': 'id'}), patch.object(api, 'wait', return_value={'metadata': {'result': {'state': {'exit_code': 1}}}}), contextlib.redirect_stdout(io.StringIO()):
            with self.assertRaises(RuntimeError):
                demo.execute(api, 'image', 'smoke', {})

    def test_agent_request_is_disposable(self):
        api = demo.API('test', 'project')
        with patch.object(api, 'request', return_value={'uuid': 'id'}) as request, patch.object(api, 'wait', return_value={'metadata': {'result': {'state': {'exit_code': 0, 'signal': -1}}}}), contextlib.redirect_stdout(io.StringIO()):
