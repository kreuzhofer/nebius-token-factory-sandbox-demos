...
def write_pdf(report, path):
    ...
    styles.add(ParagraphStyle('Receipt', fontName='ReceiptSans', fontSize=9, leading=13, spaceAfter=5))
    styles.add(ParagraphStyle('SmallReceipt', parent=styles['Receipt'], fontSize=7, leading=10))
    styles.add(ParagraphStyle('ReceiptTitle', fontName='ReceiptBold', fontSize=20, leading=25, spaceAfter=12))
    styles.add(ParagraphStyle('ReceiptHeading', fontName='ReceiptBold', fontSize=12, leading=17, spaceAfter=10))

    def p(value, small=False):
        return Paragraph(escape(str(value if value is not None else 'Unknown')).replace('\n', '<br/>'),
    ...
    for _ in range(2):
        doc = ReportDocument(str(path), pagesize=A4, leftMargin=36, rightMargin=36,
                             topMargin=36, bottomMargin=36, title='Receipt expense report')
        doc.appendix_pages = {}
        story = [Paragraph('Expense report', styles['ReceiptTitle']),
                 p('Run finished: ' + report.status.replace('_', ' '))]
        counts = Counter(entry.outcome for entry in report.entries)
        story.append(p(f'{len(report.entries)} inputs | ' + ' | '.join(
        ...
        if report.totals:
            ...
            story.append(p('No confirmed expenses. Excluded amounts may be unknown; this is not a verified zero total.'))
        story.append(p('Totals include signed refunds and exclude flagged, duplicate, and failed inputs. '
                       'Currencies are kept separate.', True))
        rows = [[p(value, True) for value in ('Input / merchant', 'Signed amount', 'Outcome / reason', 'Appendix')]]
        for entry, receipt in zip(report.entries, report.receipts):
            amount = f'{entry.currency or "?"} {entry.signed_amount or "Unknown"}'
            rows.append([p(f'{entry.source}\n{receipt.merchant or "Unknown merchant"}', True),
                         p(amount, True), p(entry.outcome + ('\n' + '; '.join(entry.reasons) if entry.reasons else ''), True),
        ...
        for entry, receipt in zip(report.entries, report.receipts):
            ...
            story.append(AppendixHeading(escape(receipt.source), styles['ReceiptHeading'], receipt.receipt_id))
            story.append(p(entry.outcome + (' - ' + '; '.join(entry.reasons) if entry.reasons else '')))
            story.append(p(f'Merchant: {receipt.merchant or "Unknown"} | Date: {receipt.transaction_date or receipt.date_text or "Unknown"}'))
            story.append(p(f'Currency: {receipt.currency or "Unknown"} | Type: {receipt.transaction_type or "Unknown"} | '
                           f'Printed total: {receipt.printed_total or "Unknown"} | Signed total: {entry.signed_amount or "Unknown"}'))
            for label, details in (('Items', receipt.line_items), ('Tax', receipt.taxes), ('Discounts', receipt.discounts)):
                if details:
                    story.append(p(label + ': ' + '; '.join(f'{item.description}: {item.amount or "Unknown"}' for item in details), True))
            if receipt.issues:
            ...
            if not receipt.rendered_pages:
                story.append(p('Original could not be rendered. ' + (receipt.error or 'No readable pages available.')))
            for page_number, rendered in enumerate(receipt.rendered_pages, 1):
                with PILImage.open(rendered) as original:
                    ...
                    for top in range(0, original.height, strip_height):
                        crop = original.crop((0, top, original.width, min(top + strip_height, original.height)))
                        buffer = BytesIO()
                        crop.save(buffer, format='PNG')
                        buffer.seek(0)
                        story.append(KeepTogether([
                            p(f'Original page {page_number}' + (' (continued)' if top else ''), True),
                            Image(buffer, width=width, height=crop.height * scale, hAlign='LEFT'),
                        ]))
