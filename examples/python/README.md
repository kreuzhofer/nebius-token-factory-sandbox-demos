...
The commands below run from this directory. From the repository root, first run
`cd examples/python`. Configuration created by `demo.py configure` lives in
...
The root pre-commit configuration and CI invoke this example's tests and Ruff
...
The receipt workflow and the small prime-number tool demo share the Python HTTPS
job helpers in this directory. Other language implementations own their own
