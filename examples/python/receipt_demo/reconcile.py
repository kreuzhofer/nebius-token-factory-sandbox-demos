...
def reconcile(receipts, decisions: Decisions):
    ...
    for receipt in receipts:
        ...
        if (
            ...
            and arithmetic.basis != "none"
            and receipt.total is not None
        ):
            base = "item" if arithmetic.basis == "items" else "subtotal"
            components = [
                c for c in arithmetic.components if c.role in (base, "discount", "tax_added", "fee")
            ]
            ...
            component_total = sum(
                (
                    (-abs(Decimal(c.amount)) if c.role == "discount" else Decimal(c.amount))
                    for c in components
                ),
                Decimal("0"),
            )
    ...
    for group in decisions.duplicates:
        ...
        for field in ("merchant", "currency", "transaction_date", "transaction_type", "total"):
            values = [
                getattr(by_id[key], field) for key in ids if getattr(by_id[key], field) is not None
            ]
            normalized = {
                Decimal(value) if field == "total" else value.strip().casefold() for value in values
            }
    ...
    for entry in entries:
        if entry.outcome == "included":
            totals[entry.currency] = totals.get(entry.currency, Decimal("0")) + Decimal(
                entry.signed_amount
            )
    status = (
        "completed_with_flags" if any(e.outcome != "included" for e in entries) else "completed"
    )
    return Report(
        status=status,
        entries=entries,
        ...
        receipts=receipts,
        artifacts={"json": "report.json", "pdf": "report.pdf"},
    )
