...
class ReceiptAgent:
    def __init__(self, models):
        self.agent = Agent(
            ...
            output_type=PromptedOutput(ExtractedDocument),
            deps_type=int,
            retries=1,
        ...
        def validate_pages(ctx: RunContext[int], document: ExtractedDocument) -> ExtractedDocument:
            if len(document.pages) != ctx.deps:
                raise ModelRetry(
                    f"Exactly {ctx.deps} source images were supplied. Return exactly "
        ...
        self.reasoner = Agent(
            models.agent,
            name="receipt_interpretation",
            output_type=ReceiptInterpretation,
            retries=1,
    ...
    async def run(self, source, work):
        ...
        try:
            ...
            stage = "extract"
            content = [
                f"Transcribe this one receipt: exactly {len(pages)} source images, in page order. "
            ...
            for number, page in enumerate(pages, 1):
                content.extend(
                    [
                        f"Page {number}",
                        BinaryContent(inference_image(page), media_type="image/jpeg"),
                    ]
                )
            log("inference.start", agent="receipt", step="extract", receipt_id=source["receipt_id"])
            result = await self.agent.run(
                content, deps=len(pages), usage_limits=UsageLimits(request_limit=2)
            )
            ...
            for name in ("merchant", "transaction_date", "currency", "transaction_type", "total"):
                if getattr(fields, name) is None:
                    fields.evidence.append(
                        Evidence(field=name, reason="Unknown or ambiguous in supplied evidence")
                    )
            receipt = Receipt(**common, **fields.model_dump())
            log(
                "receipt.done",
                receipt_id=receipt.receipt_id,
                extraction_requests=result.usage.requests,
            ...
            if isinstance(exc, ModelHTTPError) and exc.status_code in (401, 403, 404):
                ...
                raise RuntimeError(
                    f"Inference configuration failed (HTTP {exc.status_code})"
                ) from None
            # Do not print raw SDK errors: they can contain request bodies or credentials.
            error = (
                "Original could not be rendered"
                if stage == "render"
                else f"{'Interpretation' if stage == 'interpret' else 'Extraction'} did not produce a valid record within the retry limit"
            )
            log(
                "receipt.error",
                receipt_id=source["receipt_id"],
...
class ReportAgent:
    ...
    async def run(self, receipts, directory):
        ...
        agent = Agent(
            self.model,
            name="report_agent",
            output_type=ToolOutput(generate_report, name="generate_report", strict=False),
            retries=1,
            model_settings={"max_tokens": 6500, "temperature": 0},
        ...
        await agent.run(
            json.dumps(context, ensure_ascii=False), usage_limits=UsageLimits(request_limit=2)
        )
        if not generated:
    ...
    async def run(self, sources, work, output):
        ...
        try:
            ...
            agent = Agent(
                ...
                output_type=ToolOutput(process_receipts, name="process_receipts", strict=False),
                retries=0,
            ...
            remaining = max(
                1,
                float(os.environ.get("RECEIPT_DEADLINE", time.monotonic() + 3600))
                - time.monotonic(),
            )
            await asyncio.wait_for(
                agent.run(
                    "Process the supplied receipt batch and return its expense report.",
            ...
            response = {
                ...
                "artifacts": {},
                "error": f"Coordinator workflow failed ({type(exc).__name__})",
            }
