...
class API:
    ...
    def transfer(self, method, path, data=None, content_type="application/json"):
        ...
        try:
            ...
            raise RuntimeError(
                f"{method} {path}: HTTP {exc.code}; check credentials, Project header and sandbox access"
            ) from None
...
def execute(api, image, mode, env):
    operation = api.request(
        ...
        "/instances",
        {
            ...
            "networking": {"enabled": mode == "agent"},
            "timeout": 360 if mode == "agent" else 30,
            "resources_limits": {"max_layer_bytes": 268435456},
